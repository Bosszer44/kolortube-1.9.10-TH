<?php
	...
	$video_tag   = get_queried_object();
	$sidebar_pos = get_theme_mod( 'wpst_sidebar_position', 'left' );
	$ads         = array(
		'before_list' => WPS_Ads::html( 'ads_tag_page_before_list' ),
		'inside_list' => WPS_Ads::html( 'ads_tag_page_inside_list' ),
		'after_list'  => WPS_Ads::html( 'ads_tag_page_after_list' ),
	);
	add_filter(
		'theme_mod_seo_video_tag_title',
		function ( $value ) {
			// replace %%tag%%.
			$video_tag = get_queried_object();
			$value     = str_replace( '%%tag%%', $video_tag->name, $value );
			return $value;
		}
	);
	add_filter(
		'theme_mod_seo_video_tag_description',
		function ( $value ) {
			// replace %%tag%%.
			$video_tag = get_queried_object();
			$value     = str_replace( '%%tag%%', $video_tag->name, $value );
...
<div id="content">
	<?php if ( 'top' === get_theme_mod( 'seo_video_tag_position', 'top' ) ) : ?>
		<div class="hero" <?php	echo 'none' !== $sidebar_pos ? 'style="flex: 1;"' : ''; ?>>
			<div class="container" tabindex="-1">
				<div class="row hero-text">
					<div class="col-12 col-md-8 mx-auto">
						<h1><?php echo ucfirst( get_theme_mod( 'seo_video_tag_title', $video_tag->name . ' porn videos' ) ); ?></h1>
						<span class="hero-desc">
							<?php echo get_theme_mod( 'seo_video_tag_description', 'The best adult ' . $video_tag->name . ' videos' ); ?>
						</span>
					</div>
				</div>
			</div>
	...
	<?php endif; ?>
	<?php if ( 'bottom' === get_theme_mod( 'seo_video_tag_position', 'top' ) ) : ?>
		<div class="hero" <?php echo 'none' !== $sidebar_pos ? 'style="flex: 1;"' : ''; ?>>
			<div class="container" tabindex="-1">
				<div class="row hero-text">
					<div class="col-12 col-md-8 mx-auto">
